...
const InquiryCard = ({ inquiry }) => (
  ...
  >
    <p>
      <FaUser className="inline mr-2 text-blue-400" /> {inquiry.name}
    </p>
    <p>
      <FaPhone className="inline mr-2 text-green-400" /> {inquiry.mobile}
    </p>
    {/* <p><FaEnvelope className="inline mr-2 text-red-400" /> {inquiry.email}</p> */}
    {inquiry.company && (
      <p>
        <FaBuilding className="inline mr-2 text-yellow-400" /> {inquiry.company}
      </p>
    )}
  </motion.div>
);
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="w-full">
            ...
            <motion.div className="social-icon flex space-x-2 sm:space-x-4 mb-4 items-center w-full">
              {[
                { icon: <FaLinkedin />, link: "https://www.linkedin.com/school/ksrct1994/", color: "text-blue-500" },
                {
                  icon: <FaInstagram />,
          ...
          >
            ...
            <form className="space-y-4" onSubmit={handleSubmit}>
              {[
                ...
                {
                  placeholder: "Mobile",
                  icon: <FaPhone className="text-green-400 mr-2" />,
                  name: "mobile_number",
                },
                {
                  placeholder: "Email",
                  icon: <FaEnvelope className="text-red-400 mr-2" />,
                  name: "email_id",
                },
                {
                  placeholder: "Message",
                  icon: <FaMapMarkerAlt className="text-blue-400 mr-2" />,
                  name: "message",
                  isTextArea: true,
                },
              ].map((input, index) => (
